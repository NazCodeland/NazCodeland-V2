...
export class FuzzyMatcher {
	...
	protected addDistance(input: string, target: string): void {
		if (this.mapInputToTargetToDistance.has(input)) {
			...
			this.mapInputToTargetToDistance.set(
				input,
				new Map<string, number>([[target, js_levenshtein(input, target)]])
			);
		}
	}
	...
	public search(inputText: string, targetText: string): IFuzzyMatchResult[] {
		...
		const res = Array.from(inputWordSet)
	...
	public searchList(
		...
		for (const [inputIndex, inputText] of inputTextList.entries()) {
			if (!inputTextSet.has(inputText)) {
				...
				matchResultListList.push(
					this.search(inputText, targetText).map(({ distance }) => ({ distance, inputIndex }))
				);
			}
		}
		...
		return matchResultListList
			.flatMap((matchResultList) => matchResultList.filter(isWithinTolerance))
			.sort((a, b) => a.distance - b.distance);
	}

	public searchMultiList(
		inputTextList: string[],
		targetText: string,
		tolerance = 0
	): IFuzzyMultiMatchListResult[] {
...
export class TextProcessor {
	...
	static ProcessTextList(textList: string[], processors: ITextProcessor[]): string[] {
		return textList.map((text) =>
			processors.reduce((processedText, processor) => processor(processedText), text)
		);
	}
...
const titles = ['Building', 'Guild'];
const fuzzyMatcher = new FuzzyMatcher();

console.log('search');
console.log(fuzzyMatcher.search('Building', 'Build'));
console.log('searchList');
console.log(fuzzyMatcher.searchList(titles, 'Build'));
console.log('searchMultiList');
console.log(fuzzyMatcher.searchMultiList(titles, 'Build Node'));
